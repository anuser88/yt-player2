...
def show(q,frame,root):
    try:
        while True:
            ...
            tl.bind("<Button-1>", lambda e, url=f"https://youtube.com/watch?v={rec['id']}": video.createplayer(url).start())
            if rec['id']:
                ...
                lbl.image = img
                lbl.pack(pady=5)
                lbl.bind("<Button-1>", lambda e, url=f"https://youtube.com/watch?v={rec['id']}": video.createplayer(url).start())
    except queue.Empty:
        pass
